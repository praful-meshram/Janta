...
function getSiteList(){
  	...
  	url="ajaxCallOption=getSite";
  	url=url+"&sid="+Math.random();
  	
  	$.ajax({
		url:"ConversionAjaxResponse.jsp",
		type:'post',
		data:url,
		dataType:'text',
		success:function(ajaxResonse)
		{	
			 $("#siteNameDiv").html(ajaxResonse);
...
function funSearchMixture(){
	...
	if(searchItemName == "" && searchBarCode == ""){
  	...
  	url="ajaxCallOption=searchBachka";
  	...
  	$.ajax({
		...
		success:function(ajaxResonse)
		{	
			 $("#searchBachkaListDiv").html(ajaxResonse);
...
function funSearchItem(){
 ...
 // alert("search item  item_group_code "+item_group_code);
  	url="ajaxCallOption=searchItem";
  	if(searchBarCode!="" && searchBarCode!=undefined)
  	  	url=url+"&searchBarCode="+searchBarCode;
  	url=url+"&searchItemName="+searchItemName;
  	url=url+"&siteId="+siteId;
  	url=url+"&sid="+Math.random();
  	...
  	$.ajax({
		...
		success:function(ajaxResonse)
		{	
			document.getElementById("searchItemListDiv").innerHTML =ajaxResonse;
...
//function funSelectBachka(barcode,item_code,item_weight,item_name,price_version,item_mrp,item_rate,item_pv_qty,site_qty,item_qty,item_box_qty)
function funSelectBachka(barcode,item_code,item_weight,item_name,price_version,item_mrp,item_rate,item_pv_qty,site_qty,item_qty,item_box_qty,item_group_code_val){
	...
	// sssigning value to item group code
...
function funAddRow(){
	...
	//code to calculate breakdown weight total
  	...
  	if(new_wight>remaining_weight){
  		breakdown_weight=breakdown_weight-(item_weight*quantity);
  		alert("New quantity weight can nat be grater than Remaining weight. Only "+remaining_weight+" kg is remaining.");
  		return;
  	}
  ...
  */
  	...
  	if(weight_to_breakdown==0 )
  	{	
  		alert("Nothing to Mixture... "+remaining_weight+" kg is remaining ");
  		return;
  	}
...
function funSaveOrder(){
	...
	document.getElementById("searchItemListDiv").innerHTML ="";
	document.getElementById('secondWrapper2').innerHTML = "";
	document.myform.action = "Conversion.jsp";
	document.myform.submit();
}
