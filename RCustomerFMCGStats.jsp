...
<%@ page errorPage="ReportErrorPage.jsp?page=RCustomerFMCGStats.jsp" %>
...
<%
			if(hchckall.equals("1")){		
			   ...
			   fmcg = request.getParameter("fmcg");
			   criteria = custCode+"-"+custName+"-"+area+"-"+fmcg;
			   c.CustomerStatsFMCGDetail("NOTALL",custCode,custName,area,fmcg); 
%>
...
<div id="custFCMGStat" style="display: none;">
...
<%    		
			...
			while(c.rs.next()){
				...
				outputBean.setPhoneno1(phoneno1);
			...
			jsonOutputBean.getFormat().add(new jqxgridFormat("Customer Code","custCode","130px"));
			...
			jsonOutputBean.getFormat().add(new jqxgridFormat("Phone Number","phoneno1","130px"));
...
function passVar(code){
	...
	function Pass(){
		document.myform.action="RCustomerFMCGStats.jsp?Exp=1";
	    document.myform.submit();
	}
...
function check(){	
	...
	var gsonObject = jQuery.parseJSON($('#custFCMGStat').html());
		...
		$("#saveToExcel").click(function () {
               ...
               $("#custFCMGStatGrid").jqxGrid('exportdata', 'xls', 'Commission Order Report');     // not work for huge data
