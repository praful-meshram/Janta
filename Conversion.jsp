...
<%
	...
	System.out.println("=========================== Conversion JSP  ");
	...
	Integer result = mixtureMapping.saveIntoConversionHeader(b_code,loss_in_breakdown,gain_in_breakdown,siteId,userName); 
	System.out.println("after brekdown function ");

	if(result != 0){
		for(int i=0; i<itemCode.length; i++){
			if(mixtureMapping.saveConversionDetails(result,itemCode[i],Integer.parseInt(priceVersion[i]),Integer.parseInt(newQty[i]))){
				if(mixtureMapping.saveInItemMaster(itemCode[i],itemRate[i],itemMrp[i],Integer.parseInt(newQty[i]))){
					if(mixtureMapping.saveInItemPrice(itemCode[i],itemRate[i],itemMrp[i],Integer.parseInt(newQty[i]),Integer.parseInt(priceVersion[i]))){
						if(mixtureMapping.saveInItemSiteInventory(itemCode[i],siteId,Integer.parseInt(newQty[i]),Integer.parseInt(priceVersion[i]))){
							flag = true;
							System.out.println("\nUpadated\n");
						}
					}
		...
		if(flag){
			...
			mixtureMapping.setMixtureCount(break_count,b_code,siteId,price_version);
...
%>		<center><h1>Inventory saved Successfully</h3>
		...
		<b><table>
			...
			<tr>
				...
				<td>Site Name </td><td>: </td><td><%= new MixtureMapping().siteName(siteId)%></td>
			</tr>
				<tr><td></td><td> </td><td></td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
				<td>Date </td><td>: </td><td><%= new MixtureMapping().bdDate(result)%></td>
			</tr>
		</table></b>
		<table border="1">
			<thead>
...
function printPage(id){
   ...
   //window.location="PakegingForm.jsp";
   	window.location="ConversionForm.jsp";
   }
function back(){
	window.location="ConversionForm.jsp";
	}
