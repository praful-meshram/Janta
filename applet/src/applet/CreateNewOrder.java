...
public class CreateNewOrder extends JApplet implements ActionListener,
		TableModelListener, Runnable {
	TextFieldEditor tf;
	...
	JButton addButton, deleteButton, saveButton, cancelButton, clearButton,
			viewButton, holdButton, paymentButton, refreshButton;
	JCheckBox checkBox;
	RemarkColEditor remarkEditor;
	JComboBox paymentCombo, tempCombo;
	JLabel totalPrice, totalField, totalMrp, pickUp, deliver, paymentLbl,
			savingLbl, totalItemFld, pickUpFld, deliverFld, totalMrpFld,
			savingFld, emptyFld, emptyFld1, deliverInstruLbl, customerName,
			custPhone, oDate, cstCode, cstName, cstphone, cstdate,
			orderDetails, custEmpty, totalItemsLbl, customerCode, orderNumber,
			ordNumb, advance, otherDiscount, balance, balanceLblVal, blankLbl1,
			blankLbl2, otherChargesLbl, paidBalanceLbl, paidBalanceLblVal;
	JPanel tablePanel, innerbuttonPanel, buttonPanel, mainPanel, labelPanel1,
			labelPanel2, labelPanel3, labelPanel4, labelPanel5, labelPanel6,
			emptyPanel, emptyPanel1, wholeLabelPanel, custPanel, custPanel1,
			custPanel2, custPanel3;
	JTextComponent editor;
	JTextField deliverInstruText, advanceTxt, otherDiscountTxt,
			otherChargesTxt;
	Connection con = null;
	...
	String dburl = "", dbusername = "", dbpassword = "",
			statusCode = "SUBMITTED", copyOrder = "", orderType = "CREATE",
			lastOrderdate = "";
	String productValue, setProdName = "", item_code = "", pay_type = "",
			p_code = "", status = "", deliveryRemark = "";
	String user = "", disRemark = "", quantityValue = "", blankString = "",
			weightString = "", rateString = "", blank = " ", blankValue = "  ",
			blankVar = "   ", blankValue3 = "   ";
	String order_date = "", deliveryStaff = "", del_ateTime = "",
			send_DateTime, savingType = "", paymentMode = "";
	...
	int itemCount = 0, pickValue = 0, deliverCount = 0, max = 0, transId = 0,
			setQuantityFlag = 0, checkboxFlag, pickUpCnt = 0, orderNo = 0, i,
			addedRow = 0, checkQuantityVal, columnCount = 7;
	double item_mrp = 0.00f, totMRPValue = 0.0f, balanceValue = 0.0f,
			advanceValue = 0.0f, otherDiscountValue = 0.0f, savingValue = 0.0f,
			totalSaving = 0.0f, otherChargesValue = 0.0f;
	...
	public void makeGUI() {
		...
		String[] columnNames = { "Item", "Check", "Weight", "Price",
				"Quantity", "Total Price", "MRP", "Discount", "Remark",
				"ItemCode" };
		...
		remarkColwithTextField
				.setCellEditor(new DefaultCellEditor(remarkEditor));
		...
		String type = "first";
		fetchOrderValues(colwithCombo, colwithCheck, type);
		// --------
		// testUpgradedValues();
		// --------
		table.getModel().addTableModelListener(this);
	...
	public void testUpgradedValues() {
		...
		Float itemRate = 0.0f, itemMrp = 0.0f, totalPrice = 0.0f, dealOnQty = 0.0f, dealOnAmt = 0.0f, itemQty = 0.0f, qty = 0.0f, discount = 0.0f;
		try {
			String query = "select 'NEWNUM',b.item_name,b.item_code,b.item_rate,a.qty,(a.qty*b.item_rate), "
					...
					+ "from order_detail_rec a, item_master b "
					+ "where a.order_num = '"
					+ orderNo
					+ "' "
			...
			while (rsFetch.next()) {
				...
				// if(dealOnQty>0 && dealOnAmt>0){
				// itemQty=qty/dealOnQty;
				// totalPrice=totalPrice-(itemQty*dealOnAmt);
				// }
	...
	public void saveordersrec() {

		String status = "EDI";
		maxrec = mo.saveordersrec(custCode, user, status);
	}

	public void savelastrowval(int saverow) {
		try {
			float minDisAmt;
			String query1 = "SELECT deal_on_qty,deal_amount from item_master where item_code='"
					+ table.getValueAt(saverow, 9) + "'";
			item_code = table.getValueAt(saverow, 9).toString();
		...
		if (table.getValueAt(saverow, 1).toString() == "true")
			pickup_ind = 1;
		else
			pickup_ind = 0;
		double itemRate = Double.parseDouble((String) table.getValueAt(saverow,
				3));
		String itemqtystr = table.getValueAt(saverow, 4).toString();
		double itemQty = Double.parseDouble(itemqtystr);
		double totItemPrice = Double.parseDouble((String) table.getValueAt(
				saverow, 5));
		double itemMRP = Double.parseDouble((String) table.getValueAt(saverow,
				6));
		double totItemDisAmt = Double.parseDouble((String) table.getValueAt(
				saverow, 7));
		disRemark = table.getValueAt(saverow, 8).toString();
		int item_returned = 0;
		String insstack = "'" + maxrec + "','" + item_code + "','" + itemRate
				+ "'," + itemQty + "," + totItemPrice + ",'" + totItemDisAmt
				+ "','" + disRemark + "','" + itemMRP + "'," + saverow + ","
				+ pickup_ind + "," + item_returned + "";
		// System.out.println("insstack "+insstack);
		lifo.push(insstack);
		// running = true;
	}
	...
	public void run() {
		while (running) {
			...
			if (!lifo.empty()) {
				// System.out.println("In Run...............");
				String insertdata = (String) lifo.pop();
				mo.saveorderdetailrec(insertdata);
			}
		}
	}
	...
	public void deleteRow(int index) {
		...
		if (index == 0) {
			if (table.getRowCount() > 1)
				nextQuantity = table.getValueAt(table.getSelectedRow() + 1, 4)
						.toString();
		...
		if (index >= 1) {
			try {
				prevQuantity = table.getValueAt(table.getSelectedRow() - 1, 4)
						.toString();
				if (table.getSelectedRow() == 0)
					tempItem = table.getValueAt(table.getSelectedRow(), 0)
							.toString();
				else if (table.getSelectedRow() >= 1)
					tempItem = table.getValueAt(table.getSelectedRow() - 1, 0)
							.toString();
				int previousRow = -1;
				if (index != -1) {
					cme.removeAllItems();
					...
					if (previousRow >= 0 && table.getRowCount() > 0) {
						...
						// This code is added on 24/3/08 for editing purpose
						// after deleting a particular rwow.
						String str = "", temp = "";
						if (table.getRowCount() == 1) {
						...
						if (temp.length() > 0) {
							for (int i = 0; i < productArray.size(); i++) {
								...
								if (str.toUpperCase().startsWith(
										temp.toUpperCase())) {
									// cme.addItem(str); //This Statement not
									// working. Gives ArrayOutOfBounds Excp -1.
						...
						if (table.getRowCount() == 1) {
							cme.editor.setText(table.getValueAt(0, 0)
									.toString());
							...
							// cme.editor.setText(table.getValueAt(previousRow-1,
							// 0).toString());
							...
							flag = true;
							// table.changeSelection(previousRow, 4, false,
							// false);
						}
					}
					// table.addNotify();
		...
		if (temp.length() > 0) {
			for (int i = 0; i < productArray.size(); i++) {
				...
				if (str.toUpperCase().startsWith(temp.toUpperCase())) {
					// cme.addItem(str); //This Statement not working. Gives
					// ArrayOutOfBounds Excp -1.
					cme.insertItemAt(str, pos);
					pos++;
				}
			}
	...
	public void setTabFocus() {
		table = new JTable(tabModel) {
			...
			// if(statusCode.equals("DELIVERED") || statusCode.equals("TRANSIT")
			// || statusCode.equals("CHECKED") || statusCode.equals("CLOSED"))

			public boolean isCellEditable(int row, int column) {
				boolean flag = false;
				if (statusCode.equals("SUBMITTED") || statusCode.equals("HOLD")
						|| copyOrder.equals("CopyOrder")) {
					if (column == 0 || column == 4 || column == 1
							|| column == 8)
						flag = true;
					else
						flag = false;
				} else if (statusCode.equals("DELIVERED")
						|| statusCode.equals("TRANSIT")
						|| statusCode.equals("CHECKED")
						|| statusCode.equals("CLOSED"))
					flag = false;
				return flag;
			}
	...
	public void addComponents() {
		...
		try {
			rs4 = stmt
					.executeQuery("select payment_type_desc from payment_type");
			while (rs4.next()) {
		...
		innerbuttonPanel.add(refreshButton);
			...
			public void keyReleased(KeyEvent ke) {
				if ((ke.getKeyCode() == KeyEvent.VK_TAB)) {
					...
					advanceTxt.requestFocus();
						...
						&& !(ke.getKeyCode() == KeyEvent.VK_INSERT)
		...
		otherDiscountTxt.addKeyListener(new KeyAdapter() {
			public void keyReleased(KeyEvent ke) {
				if ((ke.getKeyCode() == KeyEvent.VK_TAB)) {
					...
					JOptionPane.showMessageDialog(null,
							"Negative value in not permitted", "RMS",
							JOptionPane.INFORMATION_MESSAGE);
		...
		otherChargesTxt.addKeyListener(new KeyAdapter() {
			public void keyReleased(KeyEvent ke) {
				if ((ke.getKeyCode() == KeyEvent.VK_TAB)) {
					// TAB does not work in JTable/ Applet here.Its a bug from
					// Sun Java.
				} // EOF TAB
	...
	public void setUpProductColumn(TableColumn productColumn,
		...
		try {
			...
			while (rs.next()) {
				productArray.add(rs.getString(1) + blank + " (  W : " + blank
						+ rs.getString(2) + blank + " , " + " R : "
						+ rs.getString(3) + " , " + " M : " + rs.getString(4)
						+ " ) ");
	...
	public void fetchOrderValues(TableColumn productColumn,
			TableColumn checkColumn, String type) {
		String advanceStr = "", otherDiscountStr = "", balanceStr = "", otherChargesStr = "", paidBalanceStr = "";
		try {
			String fetcQuery = "";
			// System.out.println(type);
			if (type.equals("first")) {
				fetcQuery = "select DATE_FORMAT(a.order_date,'%d-%m-%y %r'),  a.enterd_by,"
						+ " b.rate, b.qty, b.price, b.mrp, b.item_discount, b.remark, b.item_taken,"
						+ " b.item_code,d.item_name,c.custcode,c.custname,"
						+ " c.building,c.block,c.wing,c.add1,c.add2,c.phone,"
						+ " a.order_date,d.item_weight "
						+ " from orders_rec a ,"
						+ " order_detail_rec b, customer_master c, item_master d"
				...
				addedRow = 0;
				fetcQuery = "select DATE_FORMAT(a.order_date,'%d-%m-%y %r'),  a.enterd_by,"
						+ " d.item_rate, b.qty, ((d.item_rate*b.qty)-b.item_discount), d.item_mrp, b.item_discount, b.remark, b.item_taken,"
						+ " b.item_code,d.item_name,c.custcode,c.custname,"
						+ " c.building,c.block,c.wing,c.add1,c.add2,c.phone,"
						+ " a.order_date,d.item_weight "
						+ " from orders_rec a ,"
			...
			while (rsFetch.next()) {
				...
				total_taken = (int) (total_taken + item_taken);
				totalValue = totalValue + itemTotPrice;
				totalValueMRP = totalValueMRP + itemMRP;
				addedRow++;
			}// EOF While
			setOverallTotal();
			...
			totalItemFld.setText(Integer.toString(addedRow));
			totalItemFld.setForeground(Color.blue);
			...
			System.out
					.println("ArrayIndexOutOfBoundsException in fetchOrderValues"
							+ aie);
			...
			System.out
					.println("Could not connect to database in fetchOrderValues"
							+ e);
		}
	...
	public void actionPerformed(ActionEvent source) {
		...
		try {
			if (source.getSource() == (JComboBox) paymentCombo) {
				...
				// System.out.println(flagResutlt);
				if (flagResutlt == false) {
					if (statusCode.equals("SUBMITTED")
							|| statusCode.equals("HOLD")
							|| copyOrder.equals("CopyOrder")) {
						addRow();
						table.setValueAt("", table.getSelectedRow(), 4);
					} else if (statusCode.equals("DELIVERED")
				...
				if (table.getRowCount() != 0 && table.getRowCount() > 0
					...
					JOptionPane.showMessageDialog(null,
							"There are no rows anymore to delete", "RMS",
							JOptionPane.INFORMATION_MESSAGE);
				...
				if (table.getValueAt(0, 0).equals("")
						...
						&& table.getValueAt(0, 3).equals("")
						&& table.getValueAt(0, 4).equals("")) {
					emptyFlag = 1;
				}
				if (statusCode.equals("SUBMITTED") || statusCode.equals("HOLD")
						|| copyOrder.equals("CopyOrder")) {
					if (table.getRowCount() != 0 && table.getRowCount() > 0) {
						if (emptyFlag != 1) {
												...
												if (anstotalval.equals("1")) {
													...
													if (balanceValue >= 0
														...
														JOptionPane
																.showMessageDialog(
																		null,
																		"Balance amount is less than zero",
																		"RMS",
																		JOptionPane.INFORMATION_MESSAGE);
												}
											}
							...
							JOptionPane.showMessageDialog(null,
									"Please Select Item Details", "RMS",
									JOptionPane.INFORMATION_MESSAGE);
						}
					} else if (table.getRowCount() == 0)
						JOptionPane.showMessageDialog(null, "Please add a row",
								"RMS", JOptionPane.INFORMATION_MESSAGE);
				} else if (statusCode.equals("DELIVERED")
						|| statusCode.equals("TRANSIT")
						|| statusCode.equals("CHECKED")
						|| statusCode.equals("CLOSED"))
					JOptionPane.showMessageDialog(null,
							"Delivered order cannot be saved", "RMS",
							JOptionPane.INFORMATION_MESSAGE);
				...
				if (statusCode.equals("SUBMITTED") || statusCode.equals("HOLD")
					...
					if (table.getRowCount() != 0 && table.getRowCount() > 0) {
						...
						|| statusCode.equals("CLOSED"))
					JOptionPane.showMessageDialog(null,
							"Delivered order cannot be kept on hold", "RMS",
							JOptionPane.INFORMATION_MESSAGE);
			...
			} else if (source.getSource() == (JButton) refreshButton) {
				try {
					for (int i = table.getRowCount() - 1; i >= 0; i--) {
						try {
					...
					TableColumn colwithCombo = tcm.getColumn(0);
					TableColumn colwithCheck = tcm.getColumn(1);
					// System.out.println("colwithCombo"+colwithCombo+colwithCheck);
					// running = false;
	...
	public void saveOrder(String saveType) {
		/*
		 * Logic here is copyOrder=="copyOrder" then new orderNo is created both
		 * for save or hold. Else if copyOrder="" then existing orderNo ie the
		 * order which is being edited is inserted into DataBase
		 */
		max = mo.getOrderNum();
		...
		if (!msgYesNo.equals("1")) {
			...
			try {
				...
				win.eval("document.myform.orderNo.value=" + max);
	...
	public void saveInOrderTable() {
		double itemRate = 0.0f, itemQty = 0.0f, totItemPrice = 0.0f, itemMRP = 0.0f, minDisAmt = 0.0f, totOrderValue = 0.0f, totDisValue = 0.0f, totMRPValue = 0.0f;
		...
		try {
			...
			for (int i = 0; i < table.getRowCount(); i++) {
				...
				if (table.getValueAt(i, 1).toString() == "true")
					pickup_ind = 1;
				else
					pickup_ind = 0;
				itemRate = Double
						.parseDouble(table.getValueAt(i, 3).toString());
				// itemqtystr = table.getValueAt(i, 4).toString();
				itemQty = Double.parseDouble(table.getValueAt(i, 4).toString());
				totItemPrice = Double.parseDouble(table.getValueAt(i, 5)
						.toString());
				itemMRP = Double.parseDouble(table.getValueAt(i, 6).toString());
				minDisAmt = Double.parseDouble(table.getValueAt(i, 7)
						.toString());
				disRemark = table.getValueAt(i, 8).toString();
				// This Line was uncommented because total item price was coming
				// as zero

				totItemPrice = (itemQty * itemRate) - minDisAmt;
				totOrderValue = totOrderValue + totItemPrice;
				totDisValue = totDisValue + minDisAmt;
				/*mo.addOrderDetail(max, item_code, itemRate, itemQty,
						totItemPrice, minDisAmt, disRemark, itemMRP, i,
						pickup_ind, item_returned,1);
				*/
				// add site Id
				mo.addOrderDetail(max, item_code, itemRate, itemQty,
						totItemPrice, minDisAmt, disRemark, itemMRP, i,
						pickup_ind, item_returned,1,1);
			...
			// deliveryRemark = deliveryRemark +"REC("+lastOrderdate+")";
			orderType = orderType + "REC(" + lastOrderdate + ")";
			try {
				String query2 = "SELECT payment_type_code from payment_type where payment_type_desc='"
						+ pay_type + "'";
			...
			if (balanceValue < 0) {
				JOptionPane.showMessageDialog(scrollPane,
						"Balance amount is less than zero", "RMS",
						JOptionPane.INFORMATION_MESSAGE);
			} else if (balanceValue >= 0) {
				...
				checktotalval = balanceValue + advanceValue
						+ otherDiscountValue - otherChargesValue;
				// System.out.println("totOrderValue= "+totOrderValue);
				// System.out.println("checktotalval= "+checktotalval);
				// System.out.println("Math.abs(totOrderValue - checktotalval)= "+Math.abs(totOrderValue
				// - checktotalval));

				if (Math.abs(totOrderValue - checktotalval) > 1) {
					String wMessage = "Please check the Amounts. There is a difference of:"
							+ (checktotalval - totOrderValue) + ".";
					JOptionPane.showMessageDialog(scrollPane, wMessage, "RMS",
							JOptionPane.INFORMATION_MESSAGE);
					msgYesNo = "1";
				...
				if (msgYesNo.equals("0")) {
					if (copyOrder.equals("")) {
						mo.updateOrderHistory(orderNo, savingType,
								currentQuantity);
						mo.deleteOrder(orderNo);
					}

					mo.addOrders(max, custCode, totalItemsCount, pickValue,
							totOrderValue, totMRPValue, totDisValue, user,
							p_code, statusCode, deliveryRemark, orderType,
							lastOrderdate, copyOrder, order_date, balanceValue,
							advanceValue, otherDiscountValue,
							otherChargesValue, 1);
					mo.addTransactionDetails(transId, advanceValue,
							totOrderValue, balanceValue, orderNo);
					running = false;

					mo.deleteorderec(maxrec);
					mo.deleteorderec(orderNo);
				}
			}
	...
	public void setCellValues(String item_codeStr) {
		try {
			...
			String insertIntoCellQuery = "SELECT item_name,item_weight,item_rate,item_mrp, "
					+ "deal_active_flag, deal_on_qty, deal_amount "
			...
			if (rs1.next()) {
				...
				if (rs1.getString(5).equals("Y")) {
					rem = "Rs: " + rs1.getString(7) + " on " + rs1.getString(6)
							+ " QTYs ";
				}
			...
			System.out
					.println("Exception Occured while inserting data in table cells"
							+ se);
			se.printStackTrace();
		}
	}
		...
		try {
			...
			for (int row = 0; row < prodArr.size(); row++) {
				for (int j = 0; j < prodArr.size(); j++) {
					if (row != j) {
						matchString = "";
						if (prodArr.get(row).toString()
								.equalsIgnoreCase(prodArr.get(j).toString())
								&& weightArr
										.get(row)
										.toString()
										.equalsIgnoreCase(
												weightArr.get(j).toString())) {
							message = "Duplicate entry for: ";
							matchString = prodArr.get(row) + "&"
									+ weightArr.get(row);
							message = message
									+ matchString
							...
							if (n == 1) { // no
								for (int row1 = 0; row1 < table.getRowCount(); row1++) {
									if (table.getValueAt(row1, 0).equals(
											prodArr.get(row))) {
										table.changeSelection(row1, 0, false,
												false);
										String duplicateItem = table
	...
	/*
	 * public String checkDuplicateProduct(){ try{ //String retVal=""; String
	 * message = "Duplicate entry for:"; int rowCount =
	 * table.getRowCount(),flagCount=0; String
	 * prodStr1="",prodStr2="",matchString="",weightStr1="",weightStr2="";
	 * for(int row = 0; row < rowCount; row++){ prodStr1 =
	 * (String)table.getValueAt(row, 0);
	 * weightStr1=(String)table.getValueAt(row, 2); if(flagCount>0){ message =
	 * message+matchString+".\n	Do you want to allow duplicate items";
	 * //JOptionPane
	 * .showMessageDialog(null,message,"RMS",JOptionPane.INFORMATION_MESSAGE);
	 * int n=JOptionPane.showConfirmDialog(this, message,"RMS", 0,1); if(n==1){
	 * table.changeSelection(row-1, 0, false, false); String
	 * duplicateItem=table.getValueAt(row-1, 0).toString();
	 * cme.editor.setText(duplicateItem); return("0"); } else if(n==0){
	 * return("1"); }
	 * 
	 * }
	 * 
	 * for(int j = 0; j< table.getRowCount(); j++){ if(row!=j){ prodStr2 =
	 * (String)table.getValueAt(j, 0); weightStr2=(String)table.getValueAt(j,
	 * 2); if(prodStr1.equalsIgnoreCase(prodStr2) &&
	 * weightStr1.equalsIgnoreCase(weightStr2)){ flagCount++; matchString =
	 * prodStr1 + "&" +weightStr1; //message = message+matchString;
	 * //JOptionPane
	 * .showMessageDialog(null,message,"RMS",JOptionPane.INFORMATION_MESSAGE);
	 * //return("0"); } } } } return("1"); }
	 * catch(ArrayIndexOutOfBoundsException aie){
	...
	public int checkQuantity() {
		try {
			String quantityVal = (String) (table.getValueAt(
					table.getSelectedRow(), 4));
			if (quantityVal.equals("")) {
				String message = "Quantity cannot be blank";
				JOptionPane.showMessageDialog(null, message, "RMS",
						JOptionPane.INFORMATION_MESSAGE);
				return 1;
			}
	...
	public String checkPaymentType(String saveType) {
		...
		if (table.getRowCount() == 0) {
			JOptionPane.showMessageDialog(null, noItemsMsg, "RMS",
					JOptionPane.INFORMATION_MESSAGE);
			return ("0");
		}
		...
		if (saveType.equals("SAVE")) {
			/*
			 * if(paymentCombo.getSelectedItem().equals("Select Type") ||
			 * paymentCombo.getSelectedItem().equals("") &&
			 * table.getRowCount()>0){
			 * JOptionPane.showMessageDialog(null,message
			 * ,"RMS",JOptionPane.INFORMATION_MESSAGE); return("0"); }
			 */
			if (pay_type.equals("Select Type") || pay_type.equals("")
					&& table.getRowCount() > 0) {
				JOptionPane.showMessageDialog(null, message, "RMS",
						JOptionPane.INFORMATION_MESSAGE);
				return ("0");
			...
			else if (pay_type.equals("Credit") || pay_type.equals("Cash")
					|| pay_type.equals("Hawala") || pay_type.equals("Cheque")) {
				...
				if (pay_type.equals("Cash")) {
					...
					int n = JOptionPane.showConfirmDialog(this, msgconfirm,
							"RMS", 0, 1);
					if (n == 1) {
						return ("0");
					} else if (n == 0)
	...
	public String checkadv() {
		try {
			...
			if (advanceValue == 0.0) {
				...
				String message = " You have put an advance amount of "
						+ advanceValue + " . Are you sure?";

				int n = JOptionPane.showConfirmDialog(this, message, "RMS", 0,
						1);
				if (n == 1) {
					return ("0");
				} else if (n == 0) {
	...
	public String checkdis() {
		try {
			if (!otherDiscountTxt.getText().equals(""))
				otherDiscountValue = Double.parseDouble(otherDiscountTxt
						.getText());
			else
				otherDiscountValue = 0.0f;
			...
			if (otherDiscountValue == 0.0) {
				...
				String message = " You have put an discount amount of "
						+ otherDiscountValue + " . Are you sure?";
	...
	public String checktotalval() {
		try {
			...
			if (!otherDiscountTxt.getText().equals(""))
				...
				otherDiscountValue = 0.0f;
			if (!otherChargesTxt.getText().equals(""))
				otherChargesValue = Double.parseDouble(otherChargesTxt
						.getText());
			else
				otherChargesValue = 0.0f;
			balanceValue = Double.parseDouble(balanceLblVal.getText());
			...
			double checktotalval = 0.0f;
			checktotalval = balanceValue + advanceValue + otherDiscountValue
					- otherChargesValue;

			if (caltotalval != checktotalval) {
				String wMessage = "Please check the Amounts. There is a difference of:"
						+ (checktotalval - caltotalval) + ".";
				JOptionPane.showMessageDialog(scrollPane, wMessage, "RMS",
						JOptionPane.INFORMATION_MESSAGE);
				advanceTxt.setText("");
				advanceTxt.requestFocus();
							...
							if (!mousePressed)
								...
								if (s.contains("( ")) {
									...
									if (productArray.indexOf(tempStr) != -1) {
										item_codeStr = item_codeArray.get(
												productArray.indexOf(tempStr))
												.toString();
									}
									if (table.getRowCount() > 0) {
			...
			editor.addMouseListener(new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent me) {
					...
					if (table.getSelectedColumn() == 0) {
						editor.setText(table.getValueAt(table.getSelectedRow(),
						...
						if (!table.getValueAt(table.getSelectedRow(), 0)
								...
								&& !table.getValueAt(table.getSelectedRow(), 4)
										.equals("0")) {
							table.changeSelection(table.getSelectedRow(), 4,
									false, false);
						}
					}
				...
				public void keyPressed(KeyEvent ke) {
					try {
						...
						if (ke.getKeyCode() == KeyEvent.VK_TAB) {
							...
							if (table.getValueAt(table.getSelectedRow(), 0)
									.equals("") && editor.getText().equals("")) {
								JOptionPane.showMessageDialog(null,
										"Product Name is blank", "RMS",
										JOptionPane.INFORMATION_MESSAGE);
								table.changeSelection(r, 2, false, false);
									...
									&& table.getValueAt(table.getSelectedRow(),
											5).equals("")) {
								JOptionPane.showMessageDialog(null,
										"Press enter to select product name",
										"RMS", JOptionPane.INFORMATION_MESSAGE);
								table.changeSelection(r, 2, false, false);
							} else if (c == 0) {
								table.changeSelection(table.getSelectedRow(),
						...
						System.out
								.println("Exception occured in ComboBoxEdidor keyPressed");
						nfe.printStackTrace();
					}
				}// EOF KeyPressed()
				...
				public void keyReleased(KeyEvent ke) {
					...
					if ((ke.getKeyCode() == KeyEvent.VK_SHIFT)
						...
						if (table.getSelectedColumn() == 0
							...
							editor.setText(table.getValueAt(
									table.getSelectedRow(),
									table.getSelectedColumn()).toString());
						}
					}
						...
						try {
							if (editor.getText().length() < 3) {
								...
								if (!editor.getText().equals("")
									...
									splitVal = selectedTextVal
											.split(" \\(  W : ");
									editor.setText(splitVal[0]);
									if (productArray.indexOf(selectedTextVal) != -1) {
										item_codeStr = item_codeArray
												.get(productArray
														.indexOf(selectedTextVal))
												.toString();
									}
									if (table.getRowCount() > 0) {
										setCellValues(item_codeStr);
								...
								if (editor.getText().equals(prodValResult[0])) {
									item_codeStr = item_codeArray.get(
											productArray.indexOf(setProdName))
											.toString();
									if (table.getRowCount() > 0)
										setCellValues(item_codeStr);
									// ChangeSelection
									table.changeSelection(
											table.getSelectedRow(), 4, false,
											false);
							...
							System.out.println("Exception when item entered"
									+ aie);
							aie.printStackTrace();
						}
					}
					if (!(ke.getKeyCode() == KeyEvent.VK_DOWN)
						...
						try {
							...
							System.out.println("Error occured in KeyReleased"
									+ e);
						}
					...
					}// EOF Condition for up,down,pageUp, pageDown keys
						// respectively for setting the editor.
		...
		public void checkBlankRows() {
			...
			for (int row = 0; row < rowCount; row++) {
				if (table.getValueAt(row, 0).equals("")
						|| table.getValueAt(row, 2).equals("false")
						&& table.getValueAt(row, 3).equals("")
						&& table.getValueAt(row, 4).equals("")
						&& table.getValueAt(row, 5).equals("")
						&& table.getValueAt(row, 6).equals("")) {
			...
			for (int row = 0; row < rowCount; row++) {
				if (!(table.getValueAt(row, 0).equals("") || table.getValueAt(
						row, 2).equals("false")
						&& table.getValueAt(row, 3).equals("")
						&& table.getValueAt(row, 4).equals("")
						&& table.getValueAt(row, 5).equals("")
	...
	public class TextFieldEditor extends JTextField {
		...
		public TextFieldEditor() {
				...
				public void mouseReleased(MouseEvent me) {
					...
					for (int i = 0; i < rowCount; i++) {
						if (table.getValueAt(i, 0).equals("")) {
							cme.editor.setText("");
							JOptionPane.showMessageDialog(null,
									"Product cannot be blank", "RMS",
									JOptionPane.INFORMATION_MESSAGE);
							table.changeSelection(i, 0, false, false);
							break;
						} else if (table.getValueAt(i, 4).equals("")
								|| !(Double.parseDouble(table.getValueAt(i, 4)
										.toString()) >= 0.0)) {
							JOptionPane.showMessageDialog(null,
									"Quantity cannot be blank or zero", "RMS",
									JOptionPane.INFORMATION_MESSAGE);
							table.changeSelection(i, 4, false, false);
							break;
						} else if ((table.getValueAt(i, 5).equals("0.0"))
								|| (table.getValueAt(i, 5).equals("0"))) {
							String pMessage = "Total Price cannot be Zero";
							JOptionPane.showMessageDialog(scrollPane, pMessage,
									"RMS", JOptionPane.INFORMATION_MESSAGE);
							table.changeSelection(table.getSelectedRow(), 4,
									false, false);
							// return 1;
					...
					if (table.getSelectedColumn() == 0) {
						if (!table.getValueAt(table.getSelectedRow(), 0)
								...
								&& !table.getValueAt(table.getSelectedRow(), 4)
										.equals("")
								&& !(Double.parseDouble(table.getValueAt(
										table.getSelectedRow(), 4).toString()) >= 1)) {
							table.changeSelection(table.getSelectedRow(), 0,
									false, false);
						}
					} else if (table.getSelectedColumn() == 4) {
						if (!table.getValueAt(table.getSelectedRow(), 0)
							...
							|| table.getSelectedColumn() == 3
							|| table.getSelectedColumn() == 5
							|| table.getSelectedColumn() == 6) {
						table.changeSelection(table.getSelectedRow(), 0, false,
								false);
					}
				}
			...
			addKeyListener(new KeyAdapter() {
				public void keyPressed(KeyEvent ke) {
					...
					if ((ke.getKeyCode() == KeyEvent.VK_TAB && table
							.getSelectedColumn() == 4)) {
						...
						if (quantity.equals("") || quantity.equals("0")) {
							JOptionPane.showMessageDialog(null,
									"Quantity cannot be blank", "RMS",
									JOptionPane.INFORMATION_MESSAGE);
							setSelectedCol(3);// setSelected only works here
						} else if (getText().startsWith("0.0")) {
							JOptionPane.showMessageDialog(null,
									"Quantity is less than 1", "RMS",
									JOptionPane.INFORMATION_MESSAGE);
							setText("");
							setSelectedCol(3);
						} else if (!quantity.equals("")
								&& !quantity.equals("0")
								&& checkFirstColFlag == true) {
							JOptionPane.showMessageDialog(null,
									"Product Name is blank at row "
											+ (productColBlank + 1), "RMS",
									JOptionPane.INFORMATION_MESSAGE);
							setSelectedCol(3);
						} else if (!quantity.equals("")
								&& !(Double.parseDouble(quantity) >= 0.0)
								&& checkFirstColFlag == false
								&& checkLastRowFlag == false) {
							JOptionPane.showMessageDialog(null,
									"Next Row can only be added from last row",
									"RMS", JOptionPane.INFORMATION_MESSAGE);
							table.changeSelection(rowCount - 1, 0, false, false);
						} else if ((table.getValueAt(table.getSelectedRow(), 5)
								.equals("0.0"))
					...
					if ((ke.getKeyCode() == KeyEvent.VK_ALT || ke.getKeyCode() == KeyEvent.VK_N)
							|| ke.getKeyCode() == KeyEvent.VK_P) {
						table.changeSelection(table.getSelectedRow(), 4, false,
								false);
					}
		...
		public void setTotalPrice(String quantityValue) {
			try {
				double totItemPrice1 = 0.0f, itemMRP1 = 0.0f, totItemDisAmt1 = 0.0f, minDisAmt1 = 0.0f, totOrderValue1 = 0.0f, totDisValue1 = 0.0f, totMRPValue1 = 0.0f;
				double tempquantityValue = Double.parseDouble(quantityValue);
				int totDisTimes1 = 0;
				try {
					String query1 = "SELECT item_mrp, deal_amount, deal_on_qty from item_master where item_code='"
				...
				if (totDisTimes1 >= 1)
					totItemDisAmt1 = minDisAmt1 * totDisTimes1;
				else
					totItemDisAmt1 = 0;
				if (totItemDisAmt1 > 0) {
					disRemark = totItemDisAmt1 + "(@Rs " + minDisAmt1 + " per "
							+ dealQty + " QTYs)";
				}
				...
				priceValueStr = table.getValueAt(table.getSelectedRow(), 3)
						.toString();
				if (priceValueStr.equals("")) {
					JOptionPane.showMessageDialog(null, priceMessage, "RMS",
							JOptionPane.INFORMATION_MESSAGE);
				} else
					priceValue = Double.parseDouble(priceValueStr);
				if (!getText().equals("")) {
					quantityVal = Double.parseDouble(quantityValue);
					totalRate = (quantityVal * priceValue) - totItemDisAmt1;
					table.setValueAt(Double.toString(totalRate),
							table.getSelectedRow(), 5);
					// Set only MRP Price in MRP column //
					// //table.setValueAt(Double.toString(totMRPValue1),
					// table.getSelectedRow(), 6);
					table.setValueAt(
							table.getValueAt(table.getSelectedRow(), 6),
							table.getSelectedRow(), 6);
					table.setValueAt(Double.toString(totItemDisAmt1),
			...
			for (int r = 0; r < rowCount; r++) {
				...
				if (productStrValue.equals("")) {
					...
					table.changeSelection(table.getSelectedRow(), 0, false,
							false);
					break;// return flag;
				} else if (table.getValueAt(r, 4).equals("")
						|| table.getValueAt(r, 4).equals("0")
						|| table.getValueAt(r, 4).toString().startsWith("0.0")) {
					flag = true;
					JOptionPane.showMessageDialog(null,
							"Quantity is blank or zero", "RMS",
							JOptionPane.INFORMATION_MESSAGE);
					table.changeSelection(table.getSelectedRow(), 4, false,
							false);
					break;// return flag;
			...
			addKeyListener(new KeyAdapter() {
				public void keyPressed(KeyEvent ke) {
					...
					if ((ke.getKeyCode() == KeyEvent.VK_TAB && table
						...
						if (table.getSelectedRow() == rowCount - 1
								&& table.getSelectedColumn() == 8) {
							checkLastRowFlag = true;
						}
						if (!checkFirstColFlag && !checkBlankQtyFlag
								&& checkLastRowFlag) {
							if (statusCode.equals("SUBMITTED")
									|| statusCode.equals("HOLD")
									|| copyOrder.equals("CopyOrder")) {
					...
					if ((ke.getKeyCode() == KeyEvent.VK_ALT || ke.getKeyCode() == KeyEvent.VK_N)) {
						table.changeSelection(table.getSelectedRow(), 8, false,
								false);
					}
				} // EOF Pressed
		...
		try {
			...
			for (int i = 0; i < rowCount; i++) {
				...
				if (table.getValueAt(i, 6).equals("")
						|| Double
								.parseDouble(table.getValueAt(i, 6).toString()) <= 0)
					mrpVal = Double.parseDouble(table.getValueAt(i, 3)
							.toString());
				else if (!table.getValueAt(i, 6).equals("")
						|| Double
								.parseDouble(table.getValueAt(i, 6).toString()) > 0)
					mrpVal = Double.parseDouble(table.getValueAt(i, 6)
							.toString());
				if (!table.getValueAt(i, 4).equals(""))
					totMRPValue = totMRPValue
							+ Double.parseDouble((table.getValueAt(i, 4)
									.toString())) * mrpVal;
				else if (table.getValueAt(i, 4).equals(""))
				...
				if (!itemMrp.equals("")) {
					double totalmrpValue = Double.parseDouble(itemMrp
							.toString());
					totalmrpCost += totalmrpValue;
				}
				if (!itemDiscountAmt.equals("")) {
					double totalDisValue = Double.parseDouble(itemDiscountAmt
							.toString());
			...
			// This is to display totalMrp in totalMrp label and set mrp/item in
			// mrp column
	...
	// Class for adding a window popup for displaying slected products
	// description
	...
	public class DialogFrame extends Frame implements ActionListener {
		...
		public void makeDialogGUI() {
			...
			rows1 = new Vector();
			columns1 = new Vector();
			String[] columnName = { "Product", "Weight", "Price", "Quantity",
					"Total Price" };
	...
	public void setBalance() {
		try {
			if (!paymentMode.equals("Cash")) {
				...
				balanceValue = (totalFldValue - advanceValue)
						- otherDiscountValue + otherChargesValue;
				...
				if (balanceValue < 0) {
					if (result == 0)
						JOptionPane.showMessageDialog(scrollPane,
								"Balance amount cannot be less than zero",
								"RMS", JOptionPane.INFORMATION_MESSAGE);
					if (advanceValue <= totalFldValue) {
						otherDiscountTxt.setText("");
						savingFld.setText(Double.toString(Double
								.parseDouble(totalMrpFld.getText())
								- Double.parseDouble(totalField.getText())));
						balanceLblVal.setText(Double
								.toString((totalFldValue - advanceValue)
										+ otherChargesValue));
						balanceLblVal.setForeground(Color.blue);
						savingFld.setForeground(Color.blue);
					}
					...
					if (!otherDiscountTxt.getText().equals(""))
						otherDiscountValue = Double
								.parseDouble(otherDiscountTxt.getText());
					else
						otherDiscountValue = 0.0f;
					savingFld.setText(Double.toString(Double
							.parseDouble(totalMrpFld.getText())
	...
	public int setFocusOnTextField() {
		...
		try {
			...
			if (advanceValue > totalFldValue) {
				JOptionPane.showMessageDialog(null,
						"Advance amount cannot be greater than total price.",
						"RMS", JOptionPane.INFORMATION_MESSAGE);
				advanceTxt.setText("");
				advanceTxt.requestFocus();
				if (!otherDiscountTxt.getText().equals(""))
					...
					discount = 0.0f;
				balanceLblVal.setText(Double.toString(Double
						.parseDouble(totalField.getText()) - discount));
				balanceLblVal.setForeground(Color.blue);
				...
				JOptionPane.showMessageDialog(null,
						"Discount amount cannot be greater than total price.",
						"RMS", JOptionPane.INFORMATION_MESSAGE);
				otherDiscountTxt.setText("");
				otherDiscountTxt.requestFocus();
				if (!advanceTxt.getText().equals(""))
					...
					advance = 0.0f;
				balanceLblVal.setText(Double.toString(Double
						.parseDouble(totalField.getText()) - advance));
				balanceLblVal.setForeground(Color.blue);
	...
	public void processCashPayment(String item) {
		try {
			if (item.equals("Cash")) {
				...
				getValues();
				/*------------------------------------------------------------------------------------------*/
				double balanceval = (totalFldValue - advanceValue)
						- otherDiscountValue + otherChargesValue;
				balanceLblVal.setText(Double.toString(balanceval));
				balanceLblVal.setForeground(Color.blue);
				savingFld.setText(Double.toString(totalMrpLblVal
						- totalFldValue));
				savingFld.setForeground(Color.blue);
			}
		} catch (Exception e) {
		...
		try {
			...
			if (otherDiscountValue > totalFldValue) {
				JOptionPane.showMessageDialog(scrollPane,
						...
						"RMS", JOptionPane.INFORMATION_MESSAGE);
				otherDiscountTxt.setText("");
				// advanceTxt.setText(Double.toString(totalPriceVal +
				// otherChargesValue));
				otherDiscountTxt.requestFocus();
				...
				advanceTxt.setForeground(Color.blue);
				advanceTxt.setBackground(Color.white);
				advanceTxt.setText(Double.toString(totalFldValue
						- otherDiscountValue));
				...
				if (balanceval < 0)
					JOptionPane
							.showMessageDialog(
									scrollPane,
									"Balance cannot be zero. Please enter correct Discount amount.",
									"RMS", JOptionPane.INFORMATION_MESSAGE);
				...
				advanceTxt.setText(Double.toString(advanceValue
						+ otherChargesValue));
			}
		} catch (Exception e) {
	...
	public void calculateValues(double savingFldValue) {
		try {
			...
			advanceTxt.setText(Double
					.toString((totalFldValue - otherDiscountValue)
							+ otherChargesValue));

			savingFldValue = Double.parseDouble(totalMrpFld.getText())
					- Double.parseDouble(totalField.getText());
			savingFldValue = savingFldValue + otherDiscountValue;
			savingFld.setText(Double.toString(savingFldValue));
			savingFld.setForeground(Color.blue);
	...
	public void getValues() {
		try {
			...
			if (!totalMrpFld.getText().toString().equals("")) {
				totalMrpLblVal = Double.parseDouble(totalMrpFld.getText()
						.toString());
			} else
				totalMrpLblVal = 0.0f;
